...
void* runner(void* arg)
{
  ...
  char* server_buffer = (char *)malloc(256*sizeof(char));
  read((*(struct client_sock *)arg).sock , buffer , 256) ;
  printf("client %d : %s\n",htons((*(struct client_sock *)arg).address.sin_port),buffer );
  fgets(server_buffer,256,stdin);
  write((*(struct client_sock *)arg).sock , server_buffer , 256);
...
int main(){
  ...
  while(1)
  {
    ...
    pthread_t thread;
    pthread_create(&thread, NULL, runner,(void *)cli);
  }
  return 0;
}
